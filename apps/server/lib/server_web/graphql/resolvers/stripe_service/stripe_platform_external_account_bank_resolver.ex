defmodule ServerWeb.GraphQL.Resolvers.StripeService.StripePlatformExternalAccountBankResolver do
  ...
  @spec list_by_stripe(any, %{atom => any}, %{context: %{current_user: User.t()}}) :: success_list() | empty_list() | error_tuple()
    ...
    case current_user.role do
      true ->
        case Repo.get_by(StripeAccount, %{user_id: current_user.id}) do
          ...
          account ->
            with {:ok, struct} <- StripePlatformExternalAccountBankService.list(:bank_account, %{account: account.id_from_stripe}) do
              {:ok, struct}
            else
              failure ->
                case failure do
  ...
  def list_by_stripe(_parent, _args, _info), do: {:ok, %{error: "unauthenticated"}}
