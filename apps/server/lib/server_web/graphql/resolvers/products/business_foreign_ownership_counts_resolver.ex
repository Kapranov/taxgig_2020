defmodule ServerWeb.GraphQL.Resolvers.Products.BusinessForeignOwnershipCountsResolver do
  @moduledoc """
  The BusinessForeignOwnershipCount GraphQL resolvers.
  """
  ...
  @type t :: BusinessForeignOwnershipCount.t()
  @type reason :: any
  @type ok :: {:ok}
  ...
  def list(_parent, _args, %{context: %{current_user: current_user}}) do
    if is_nil(current_user) do
      ...
      data = Services.list_business_foreign_ownership_count()
      {:ok, data}
    end
  end
  ...
  def show(_parent, %{id: id}, %{context: %{current_user: current_user}}) do
    if is_nil(id) || is_nil(current_user) do
      ...
      try do
        data = Services.get_business_foreign_ownership_count!(id)
        {:ok, data}
      rescue
        Ecto.NoResultsError ->
          {:error, "The BusinessForeignOwnershipCount #{id} not found!"}
      end
    end
  end
  ...
  def create(_parent, args, %{context: %{current_user: current_user}}) do
    if is_nil(current_user) do
      ...
      args
      |> Services.create_business_foreign_ownership_count()
      |> case do
        {:ok, data} ->
          {:ok, data}
        {:error, changeset} ->
  ...
  @spec update(any, %{id: bitstring, business_foreign_ownership_count: map()}, %{context: %{current_user: User.t()}}) :: result()
  def update(_parent, %{id: id, business_foreign_ownership_count: params}, %{context: %{current_user: current_user}}) do
    if is_nil(id) || is_nil(current_user) do
      {:error, [[field: :id, message: "Can't be blank or Permission denied for current_user to perform action Update"]]}
    else
      try do
        Repo.get!(BusinessForeignOwnershipCount, id)
        |> BusinessForeignOwnershipCount.changeset(params)
        |> Repo.update
      rescue
        Ecto.NoResultsError ->
          {:error, "The BusinessForeignOwnershipCount #{id} not found!"}
  ...
  def update(_parent, _args, _info) do
    {:error, [[field: :current_user,  message: "Unauthenticated"], [field: :id, message: "Can't be blank"], [field: :business_foreign_ownership_count, message: "Can't be blank"]]}
  end
