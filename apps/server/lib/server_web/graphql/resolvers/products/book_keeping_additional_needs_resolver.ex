defmodule ServerWeb.GraphQL.Resolvers.Products.BookKeepingAdditionalNeedsResolver do
  @moduledoc """
  The BookKeepingAdditionalNeed GraphQL resolvers.
  """
  ...
  @type t :: BookKeepingAdditionalNeed.t()
  @type reason :: any
  @type ok :: {:ok}
  ...
  def show(_parent, %{id: id}, %{context: %{current_user: current_user}}) do
    if is_nil(id) || is_nil(current_user) do
      ...
      try do
        data = Services.get_book_keeping_additional_need!(id)
        {:ok, data}
      rescue
        Ecto.NoResultsError ->
  ...
  def create(_parent, args, %{context: %{current_user: current_user}}) do
    if is_nil(current_user) do
      ...
      args
      |> Services.create_book_keeping_additional_need()
      |> case do
        {:ok, data} ->
          {:ok, data}
        {:error, changeset} ->
  ...
  @spec update(any, %{id: bitstring, book_keeping_additional_need: map()}, %{context: %{current_user: User.t()}}) :: result()
  def update(_parent, %{id: id, book_keeping_additional_need: params}, %{context: %{current_user: current_user}}) do
    if is_nil(id) || is_nil(current_user) do
  ...
  def update(_parent, _args, _info) do
    {:error, [[field: :current_user,  message: "Unauthenticated"], [field: :id, message: "Can't be blank"], [field: :book_keeping_additional_need, message: "Can't be blank"]]}
  end
