defmodule ServerWeb.GraphQL.Resolvers.Products.BookKeepingAnnualRevenuesResolver do
  @moduledoc """
  The BookKeepingAnnualRevenue GraphQL resolvers.
  """
  ...
  @type t :: BookKeepingAnnualRevenue.t()
  @type reason :: any
  @type ok :: {:ok}
  ...
  def show(_parent, %{id: id}, %{context: %{current_user: current_user}}) do
    if is_nil(id) || is_nil(current_user) do
      ...
      try do
        data = Services.get_book_keeping_annual_revenue!(id)
        {:ok, data}
      rescue
        Ecto.NoResultsError ->
          {:error, "The BookKeepingAnnualRevenue #{id} not found!"}
      end
    end
  end
  ...
  def create(_parent, args, %{context: %{current_user: current_user}}) do
    if is_nil(current_user) do
      ...
      args
      |> Services.create_book_keeping_annual_revenue()
      |> case do
        {:ok, data} ->
          {:ok, data}
        {:error, changeset} ->
  ...
  @spec update(any, %{id: bitstring, book_keeping_annual_revenue: map()}, %{context: %{current_user: User.t()}}) :: result()
  def update(_parent, %{id: id, book_keeping_annual_revenue: params}, %{context: %{current_user: current_user}}) do
    if is_nil(id) || is_nil(current_user) do
      {:error, [[field: :id, message: "Can't be blank or Permission denied for current_user to perform action Update"]]}
    else
      try do
        Repo.get!(BookKeepingAnnualRevenue, id)
        |> BookKeepingAnnualRevenue.changeset(params)
        |> Repo.update
      rescue
        Ecto.NoResultsError ->
          {:error, "The BookKeepingAnnualRevenue #{id} not found!"}
  ...
  def update(_parent, _args, _info) do
    {:error, [[field: :current_user,  message: "Unauthenticated"], [field: :id, message: "Can't be blank"], [field: :book_keeping_annual_revenue, message: "Can't be blank"]]}
  end
