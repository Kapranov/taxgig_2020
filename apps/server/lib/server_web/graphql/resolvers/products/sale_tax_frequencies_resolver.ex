defmodule ServerWeb.GraphQL.Resolvers.Products.SaleTaxFrequenciesResolver do
  @moduledoc """
  The SaleTaxFrequency GraphQL resolvers.
  """
  ...
  def show(_parent, %{id: id}, %{context: %{current_user: current_user}}) do
    if is_nil(id) || is_nil(current_user) do
      ...
      try do
        data = Services.get_sale_tax_frequency!(id)
        {:ok, data}
      rescue
        Ecto.NoResultsError ->
          {:error, "The SaleTaxFrequency #{id} not found!"}
      end
    end
  end
  ...
  def create(_parent, args, %{context: %{current_user: current_user}}) do
    if is_nil(current_user) do
      ...
      args
      |> Services.create_sale_tax_frequency()
      |> case do
        {:ok, data} ->
          {:ok, data}
        {:error, changeset} ->
  ...
  @spec update(any, %{id: bitstring, sale_tax_frequency: map()}, %{context: %{current_user: User.t()}}) :: result()
  def update(_parent, %{id: id, sale_tax_frequency: params}, %{context: %{current_user: current_user}}) do
    if is_nil(id) || is_nil(current_user) do
      {:error, [[field: :id, message: "Can't be blank or Permission denied for current_user to perform action Update"]]}
    else
      try do
        Repo.get!(SaleTaxFrequency, id)
        |> SaleTaxFrequency.changeset(params)
        |> Repo.update
      rescue
        Ecto.NoResultsError ->
          {:error, "The SaleTaxFrequency #{id} not found!"}
  ...
  def update(_parent, _args, _info) do
    {:error, [[field: :current_user,  message: "Unauthenticated"], [field: :id, message: "Can't be blank"], [field: :sale_tax_frequency, message: "Can't be blank"]]}
  end
