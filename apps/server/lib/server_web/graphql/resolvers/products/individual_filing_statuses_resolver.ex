defmodule ServerWeb.GraphQL.Resolvers.Products.IndividualFilingStatusesResolver do
  @moduledoc """
  The IndividualFilingStatuse GraphQL resolvers.
  """
  ...
  @type t :: IndividualFilingStatus.t()
  @type reason :: any
  @type ok :: {:ok}
  ...
  def show(_parent, %{id: id}, %{context: %{current_user: current_user}}) do
    if is_nil(id) || is_nil(current_user) do
      ...
      try do
        data = Services.get_individual_filing_status!(id)
        {:ok, data}
      rescue
        Ecto.NoResultsError ->
          {:error, "The IndividualFilingStatus #{id} not found!"}
      end
    end
  end
  ...
  def create(_parent, args, %{context: %{current_user: current_user}}) do
    if is_nil(current_user) do
      ...
      args
      |> Services.create_individual_filing_status()
      |> case do
        {:ok, data} ->
          {:ok, data}
        {:error, changeset} ->
  ...
  @spec update(any, %{id: bitstring, individual_filing_status: map()}, %{context: %{current_user: User.t()}}) :: result()
  def update(_parent, %{id: id, individual_filing_status: params}, %{context: %{current_user: current_user}}) do
    if is_nil(id) || is_nil(current_user) do
      {:error, [[field: :id, message: "Can't be blank or Permission denied for current_user to perform action Update"]]}
    else
      try do
        Repo.get!(IndividualFilingStatus, id)
        |> IndividualFilingStatus.changeset(params)
        |> Repo.update
      rescue
        Ecto.NoResultsError ->
          {:error, "The IndividualFilingStatus #{id} not found!"}
  ...
  def update(_parent, _args, _info) do
    {:error, [[field: :current_user,  message: "Unauthenticated"], [field: :id, message: "Can't be blank"], [field: :individual_filing_status, message: "Can't be blank"]]}
  end
