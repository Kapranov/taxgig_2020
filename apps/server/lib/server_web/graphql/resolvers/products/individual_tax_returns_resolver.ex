defmodule ServerWeb.GraphQL.Resolvers.Products.IndividualTaxReturnsResolver do
  ...
  @spec update_for_admin(any, %{id: bitstring, individual_tax_return: map()}, %{context: %{current_user: User.t()}}) :: result()
  def update_for_admin(_parent, %{id: id, individual_tax_return: params}, %{context: %{current_user: current_user}}) do
    if current_user.admin do
      try do
        Repo.get!(IndividualTaxReturn, id)
        |> IndividualTaxReturn.changeset(params)
