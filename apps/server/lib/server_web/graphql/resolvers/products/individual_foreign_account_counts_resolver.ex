defmodule ServerWeb.GraphQL.Resolvers.Products.IndividualForeignAccountCountsResolver do
  @moduledoc """
  The IndividualForeignAccount GraphQL resolvers.
  """
  ...
  @type t :: IndividualForeignAccountCount.t()
  @type reason :: any
  @type ok :: {:ok}
  ...
  def show(_parent, %{id: id}, %{context: %{current_user: current_user}}) do
    if is_nil(id) || is_nil(current_user) do
      ...
      try do
        data = Services.get_individual_foreign_account_count!(id)
        {:ok, data}
      rescue
        Ecto.NoResultsError ->
          {:error, "The IndividualForeignAccountCount #{id} not found!"}
      end
    end
  end
  ...
  def create(_parent, args, %{context: %{current_user: current_user}}) do
    if is_nil(current_user) do
      ...
      args
      |> Services.create_individual_foreign_account_count()
      |> case do
        {:ok, data} ->
          {:ok, data}
        {:error, changeset} ->
  ...
  @spec update(any, %{id: bitstring, individual_foreign_account_count: map()}, %{context: %{current_user: User.t()}}) :: result()
  def update(_parent, %{id: id, individual_foreign_account_count: params}, %{context: %{current_user: current_user}}) do
    if is_nil(id) || is_nil(current_user) do
      {:error, [[field: :id, message: "Can't be blank or Permission denied for current_user to perform action Update"]]}
    else
      try do
        Repo.get!(IndividualForeignAccountCount, id)
        |> IndividualForeignAccountCount.changeset(params)
        |> Repo.update
      rescue
        Ecto.NoResultsError ->
          {:error, "The IndividualForeignAccountCount #{id} not found!"}
  ...
  def update(_parent, _args, _info) do
    {:error, [[field: :current_user,  message: "Unauthenticated"], [field: :id, message: "Can't be blank"], [field: :individual_foreign_account_count, message: "Can't be blank"]]}
  end
