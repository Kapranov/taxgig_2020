defmodule ServerWeb.GraphQL.Integration.Products.IndividualForeignAccountCountIntegrationTest do
  ...
  describe "#show" do
    it "returns specific IndividualForeignAccountCount by role's Tp" do
      ...
      query = """
      {
        showIndividualForeignAccountCount(id: \"#{struct.id}\") {
          id
          name
          individual_tax_returns {
            id
  ...
  describe "#find" do
    it "returns specific IndividualForeignAccountCount by role's Tp" do
      ...
      query = """
      {
        findIndividualForeignAccountCount(id: \"#{struct.id}\") {
          id
          name
          individual_tax_returns {
            id
  ...
  describe "#update" do
    it "updated specific IndividualForeignAccountCount by role's Tp" do
      user = insert(:tp_user)
      individual_tax_return = insert(:tp_individual_tax_return, %{user: user})
      struct = insert(:tp_individual_foreign_account_count, %{name: "1", individual_tax_returns: individual_tax_return})
  ...
  describe "#delete" do
    it "delete specific IndividualForeignAccountCount" do
      ...
      struct = insert(:individual_foreign_account_count, %{individual_tax_returns: individual_tax_return})

      mutation = """
      {
        deleteIndividualForeignAccountCount(id: \"#{struct.id}\") {id}
      }
      """
  ...
  describe "#dataloads" do
    it "created IndividualForeignAccountCount" do
      user = insert(:user)
      individual_tax_return = insert(:individual_tax_return, %{user: user})
      %{id: id} = insert(:individual_foreign_account_count, %{individual_tax_returns: individual_tax_return})
      ...
      loader =
        ...
        |> Dataloader.load(:individual_foreign_account_counts, Core.Services.IndividualForeignAccountCount, id)
        |> Dataloader.run

      data = Dataloader.get(loader, :individual_foreign_account_counts, Core.Services.IndividualForeignAccountCount, id)
